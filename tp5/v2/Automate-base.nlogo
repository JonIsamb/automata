...
to setup-player
  create-turtles 1 [
    ...
    set current-state "stop"  ; Fixed: changed from "player-stop" to "stop"
    set next-state "stop"     ; Fixed: changed from "player-stop" to "stop"
    set stack []
...
to process-player-stop
  ...
  if button-J [
    transition-to "jump" "J" []  ; Fixed: changed from "do-jump" to "jump"
  ]
...
to process-do-jump
  ...
  transition-to "stop" "ε" []  ; Fixed: changed from "player-stop" to "stop"
end
...
; Add key handling - Fixed key mappings
to __key-down [ key ]
  if key = "g" or key = "G" [ press-avancer ]
  if key = "a" or key = "A" [ press-player-stop ]
  if key = "n" or key = "N" [ press-north ]
  if key = "e" or key = "E" [ press-east ]
  if key = "s" or key = "S" [ press-south ]
  if key = "w" or key = "W" [ press-west ]
  if key = "j" or key = "J" [ press-do-jump ]
  if key = "p" or key = "P" [ press-pickup ]
  if key = "d" or key = "D" [ press-drop ]
  if key = "q" or key = "Q" [ press-quit ]
end
